...
# При уже работающем снимке SQLite: если за один опрос «аномальных» строк с заполненной колонкой команды
# больше порога — нет ключа в БД или в SQLite сохранена пустая команда при непустой ячейке (часто после 429).
# Тогда не шлём MAX, только дописываем снимок; если ключ есть и непустая сохранённая команда отличается от таблицы — шлём как обычно.
# 0 = выключить (старое поведение).
