...
# При уже работающем снимке SQLite: если за один опрос строк с заполненной колонкой команды,
# которых ещё нет в БД, больше этого числа — считаем «догон после сбоя ключей/очистки», не шлём MAX,
# только записываем текущие значения в снимок. Изменение команды у строки УЖЕ из снимка по-прежнему шлётся.
# 0 = выключить (старое поведение: каждая «новая для SQLite» строка может вызвать отправку).
CATCHUP_SILENT_MISSING_ROWS_THRESHOLD: int = max(
    0,
    int(os.getenv("CATCHUP_SILENT_MISSING_ROWS_THRESHOLD", "15") or "15"),
)
...
# Альтернативные имена вкладок в том же порядке (частые сокращения в книгах).
MONTH_SHEET_SHORT_NAMES: tuple[str, ...] = (
    "Янв",
    "Февр",
    "Март",
    "Апр",
